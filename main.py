...
def get_exp(cur_id):
    ...
    return (
    {'name': cur_id_name, 'par': cur_par, 'timecourse': cur_timecourse[mask], 'time_point': cur_time_point[mask]})
...
def log10Kd_to_K(theta):
    ...
    return pd.concat([Ks, theta[np.setdiff1d(theta.index,
                                             ["logK_on_TN", "logK_on_TC", "logK_on_RN", "logK_on_RC", "logK_D_TN",
                                              "logK_D_TC", "logK_D_RN", "logK_D_RC"])]])


def log10Kd_to_K_fixed(theta):
    th = pd.Series({'logK_on_TC': 7.924122834399438, 'logK_on_TN': 8.886452489928546, 'logK_on_RN': 10.505153641111766,
                    'logK_on_RC': 7.397875217995472, 'm_alpha': 0.0011, 'alpha0': -0.39})
    theta["m_alpha"] = 0.0011
    Ks = pd.Series([pd.to_numeric(10 ** theta["logK_on_TN"]),
                    pd.to_numeric(10 ** theta["logK_on_TC"]),
                    pd.to_numeric(10 ** theta["logK_on_RN"]),
                    pd.to_numeric(10 ** theta["logK_on_RC"]),
...
def K_to_log10Kd(theta):
    ...
    logKs.index = ["logK_on_TN", "logK_on_TC", "logK_on_RN", "logK_on_RC", "logK_off_TN", "logK_off_TC", "logK_off_RN",
                   "logK_off_RC"]
    return (pd.concat([logKs, theta[np.setdiff1d(theta.index,
                                                 ["K_on_TN", "K_on_TC", "K_on_RN", "K_on_RC", "K_D_TN", "K_D_TC",
                                                  "K_D_RN", "K_D_RC"])]]))
